...
Vector.prototype.multiplyScalar = function( scalar ) { this.x *= scalar; this.y *= scalar; return this; };
...
function SmartCanvas(options, parentElement){
	...
	if( parentElement === window ) document.body.appendChild( this.canvas );
	...
	this.canvas.addEventListener( 'mousemove', this.pointerEvent.bind( this, 'over' ) );
	this.canvas.addEventListener( 'click', this.pointerEvent.bind( this, 'click' ) );
}
(function(){
  this.pointerEvent = function (event, e){
    ...
    for(i in contents){
      ...
      if(item[event] && typeof item[event] == 'function'){
        item[event](this.toItemSpace(item, camSpacePoint), e);
      }
    }
  };
	...
	this.toWorldSpace = function toWorldSpace(screenPoint){
		var camPoint = new Vector(screenPoint.x, screenPoint.y);
